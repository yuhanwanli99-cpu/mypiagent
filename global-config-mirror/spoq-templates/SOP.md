...
① Router（flash×3）→ trivial? → 主代理直接做，不进流水线
   ...
   │
   ▼
② 需求分析师（写需求.md 初稿，含数据缺口清单 GAP1..N）
   → 主代理按缺口拆成多个检索员并行（每个检索员只填 1-3 个缺口）
   → 缺口未闭合 → 主代理再派下一批检索员（按剩余缺口重拆）
   需求.md 完成（缺口全闭合 或 剩余全标 unfilled+检索词+原因）
   │
🚪 Gate1 人工：确认 / 打回
   │
   ▼
③ 方案研究员（查 prior art，产对比表草稿）→ 架构师（选型选项+架构.md）   ← 缺口驱动迭代
   架构.md 完成（每模块tier标注）
   │
...
- 文件名固定：`requirement.md`（需求）、`search-report.md`（检索报告）、`architecture.md`（架构）、`solution-comparison.md`（方案对比）、`contract.md`（契约）、`test-cases.md`（测试用例）、`audit-report.md`（一致性报告）。
- 检索员等调研角色**只追加自己的 findings 到 `search-report.md`**，禁止整文件覆盖（多检索员并行时合并由主代理负责，或各自写 `search-report-{gapid}.md` 分片，主代理汇合）。
...
6. 调研/查证阶段（②③）：**按缺口拆批派发**——需求分析师产出缺口清单后，把 GAP1..N 分成几组，每组派一个检索员（每个只填 1-3 个缺口），同一轮并行发出。缺口未闭合 → 按剩余缺口重拆再派一批（不重派已闭合的）。禁止把"合并+对账+构建核实"全塞给一个 agent。
7. 合并职责在主代理：多个检索员分片产出（`search-report-{gapid}.md`）→ 主代理机械汇合成 `search-report.md`（格式操作，不做内容判断）。子代理完成的通知是 `<task-notification>`，不要等轮询。
8. 任务完成 / 用户终止 → 在对话中清空自己的"进行中"认知，不残留。
9. 规模异常（架构师拆出任务数远超 Router 预估）→ 停下向用户展示，问是否降级/分批。
